...
The `tinyoperations` R package also adds a few other things:
 ...
 - The `lock_TF()` function locks `T` and `F` to prevent the user from re-assigning them. Removing the `T` and `F` objects allows re-assignment again.
 - The `X %<-c% A` operator creates a `CONSTANT` `X` with assignment `A`. Constants cannot be changed, only accessed or removed. So if you have a piece of code that absolutely requires some unchangeable constant, use this operator to create said constant.
