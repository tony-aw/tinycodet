...
So here `tinyoperator`'s `import_as()` function can come to the rescue. Below is an example where `dplyr` is loaded, along with some dependencies, and `powerjoin` (which is an extension), all under one alias which I'll call `dr`.
