...
The `tidyoperators` R package adds 2 additional matrix operators:
 ...
 - The `x %row~% rank` operator re-orders the elements of every row of matrix `x` by the rank given in matrix `rank`.
 - The `x %col~% rank` operator re-orders the elements of every column of matrix `x` by the rank given in matrix `rank`.
...
Here a practical example of their usage when re-ordering strings, words, or sentences (notice also the usage the `%row~%` operator, which allows for slightly faster re-ordering) :
...
Currently, `stri_locate_ith` internally uses `mapply` for the vectorization. Once this R package is out of the `experimental` phase, I plan to replace `mapply` with a `C++` loop, though `stri_locate_ith` is currently already quite fast.
...
Another string operator is `x %ss%  s`. This essentially splits character vector `x` into a vector containing only individual characters; then this vector is subset-ted by the number given in numeric vector `s`. Honestly, I do not think one would need this often, but it can be handy sometimes. For example:
