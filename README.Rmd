...
For example, exposes the infix operators from the `tidytable` and `data.table` R packages to the (global) environment:
...
For proper programming etiquette, I also highly recommend the following R packages:
 ...
 - The `import` R package, which allows attaching packages **locally**. For example: attaching a package **only** inside a function environment, without affecting general namespace outside of the function environment.
