...
Suppose one wants to transform the first vowels in the strings of a character vector `x` such that all upper case vowels become lower case, and vice-versa.
...
To transform **only** the **second-last** occurrence, one can now use `stri_locate_ith()` in a very similar way as was done with `stri_locate_first/last`:
...
The `tidyoperators` R-package includes the following fully vectorized "substr-" functions:
