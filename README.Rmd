...
The string arithmetic and sub-setting operators and functions given so far can do a lot, but it's not always flexible enough. To add extra flexibility, there is also the `s_strapply(x, fun, w=F, clp="", ...)` function. This function applies the following steps to every element (every string) of character vector x:
