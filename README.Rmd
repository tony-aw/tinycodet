...
Just to clarify: I am not claiming this import system provided in the `tinyoperators` package is always the best system or whatever. This is just another option I lovingly share with R users, just like the `import` and `box` packages provide additional options. Please feel free to completely ignore this section if you really prefer using `library()` :-)
...
So here `tinyoperator`'s `import_as()` function can come to the rescue. Below is an example where `dplyr` is loaded, and `powerjoin` (which is an extension), all under one alias which I'll call `dr`.
