...
CHANGELOG (EXPERIMENTAL VERSIONS):
 ...
 - 28 June 2023: renamed the `depends` and `extends` arguments of `import_as()` to `dependencies` and `extenions`, to avoid confusion.
...
So here `tinyoperator`'s `import_as()` function can come to the rescue. Below is an example where `dplyr` is loaded, along with its dependencies, and `powerjoin` (which is an extension), all under one alias which I'll call `dr`.
