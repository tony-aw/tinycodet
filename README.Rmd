...
The main arguments of the `import_as()` function are:
 ...
 - `alias`: the name (unquoted) of the alias under which to load the main package, and any specified (reverse) dependencies. To keep aliases easily distinguished from other objects that can also be subset with the `$` operator, I recommend starting all alias names with a dot (`.`).
...
The `import_as()` function will first load the dependencies in the order specified in argument `dependencies` if given (so the order matters). Then it loads the package named in argument `package`. And then it loads the extensions in the order specified in argument `extensions` if given (again, the order matters).
