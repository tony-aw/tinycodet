...
#' This argument is only used when \code{action = "expose"}.
...
#' The \code{exclude, include.only, overwrite, inherits} arguments
#' are only used when \code{action = "expose"}. \cr
...
#'
#' @seealso [import_inops()], [tinyoperations_import()]
#'
#'
#'
#'
...
import_inops.control <- function(
  ...
  if(!is.null(exclude)) {
    if(!is.character(exclude) | any(!nzchar(exclude))) {
      stop("`exclude` must be a character vector with function names")
    }
  }
  if(!is.null(include.only)) {
    if(!is.character(include.only) | any(!nzchar(include.only))) {
      stop("`include.only` must be a character vector with function names")
    }
  }
