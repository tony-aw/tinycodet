...
#' \cr
#' The objects to be selected can be specified using any combination of the following: \cr
#' \itemize{
...
#' The user can also specify a \bold{named} character vector here.
#' In that case the objects will also be renamed to the given names
#' when exposed to the current environment.
...
source_selection <- function(
  ...
  if(!is.null(select)) {
    ...
    if(is.null(names(select)))names(select) <- select
    ind <- !nzchar(names(select))
    if(length(ind)>0){names(select)[ind] <- select[ind]}
    for (i in 1:length(select)) {
      attr(tempenv[[select[i]]], "env") <- tempenv
      assign(names(select)[i], tempenv[[select[i]]], envir = parent.frame(n = 1))
    }
  }
