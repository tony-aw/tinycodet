...
The `x %s-% p` and `x %s/% p` operators (and their in-place equivalents, given later), and the string detection operators (`%s{}%`, `%s!{}%`) perform pattern matching for various purposes. When a character vector or string is given on the right hand side, this is interpreted as case-sensitive `regex` patterns from `stringi`.
