...
What `tinyoperations` attempts to do with its import system, is to somewhat find the best of both worlds. Basically, `tinyoperations` has functions that allow the following import functionality lacking in base R:
 ...
 - Allow package + its re-exports + its dependencies + its enhances + its extensions to be loaded under **one alias**. This essentially combines the attaching advantage of using multiple related packages (row 7 in table above), whilst keeping most advantages of aliasing a package.
...
Therefore, `tinyoperations` also adds the `import_inops()` function, which exposes the infix operators as specified in either an alias object, or from a character vector of package names. The infix operators are exposed to the current environment (like the global environment, or the environment within a function), but does not attach the functions to the namespace.
...
Exposing and unexposing can, as stated earlier, also done using a character vector of package names. (Note that the order of the packages matters whenever there's a conflict: operators from later named packages overwrite those from earlier named packages).

The following code exposes the infix operators from the [data.table](https://github.com/Rdatatable/data.table) and [tidytable](https://github.com/markfairbanks/tidytable) R packages:

```{r}
import_inops(expose = c("data.table", "tidytable"))
```

And similarly one can remove the exposed infix operators again from the current environment:

```{r}
import_inops(unexpose = c("data.table", "tidytable"))
```

When unexposing, the order of the package names do not matter.
...
import_inops(unexpose = tdt.)
import_inops(expose = c("data.table", "tidytable"), , include.only = ":=")
import_inops(unexpose = c("data.table", "tidytable"))
```
