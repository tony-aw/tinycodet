...
vignette: >
  %\VignetteIndexEntry{The import system}
...
When importing packages with `tinyoperations`' import system, S3 methods will work just fine. For example, the S3 method " `plot()` " works on `mgcv` even when loaded in an alias:
