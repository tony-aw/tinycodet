...
First: The decimal number truth testing operators assume that if the absolute difference between `x` and `y` is smaller than the Machine tolerance, `sqrt(.Machine$double.eps)`, then `x` and `y` ought to be consider to be equal.

Second: The decimal number (in)equality operators do NOT allow vector recycling. When comparing any two numbers `x` and `y`, they must be of equal length, or one of them must be a scalar (i.e. length of 1). Otherwise, an error is returned.
...
tinytest::expect_error(
  1:2 %d==% 1:3 # recycling not allowed for your own safety, thus gives error
)
...
Only the "class" attribute is removed when converting with these functions. 
...
One can re-assign the values `T` and `F`. One can even say `T <- FALSE` and `F <- TRUE`!. `tinyoperations` adds the `lock_TF()` function that forces `T` to stay `TRUE` and `F` to stay `FALSE`. Essentially, the `lock_TF()` function locks `T` and `F` to `TRUE` and `FALSE`, to prevent the user from re-assigning them. Removing the created `T` and `F` constants allows re-assignment again.
