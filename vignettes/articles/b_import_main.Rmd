...
What `tinycodet` attempts to do with its import system, is to somewhat find the best of both worlds. Basically, `tinycodet` has functions that allow the following import functionality lacking in base R:
 ...
 - Allow an R package + its re-exports + its dependencies + its extensions to be imported under **one alias**. This essentially combines the attaching advantage of using multiple related packages (row 7 in table above), whilst keeping most advantages of using without attaching a package.
...
The main arguments of the `import_as()` function are:
 ...
 - `extensions`: an optional character vector giving the extensions/reverse-dependencies of the `main_package` to import under the same alias also.
