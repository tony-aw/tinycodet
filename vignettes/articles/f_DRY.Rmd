...
Here a conditional subset of the object `object` is transformed where the condition is using a function referring to `object` itself. Consequently, reference to `object` is written 4 times! This can become cumbersome quickly. Notice also that the above code gives an unnecessary warning, due to `ifelse()` requiring the entirity of `log(object)`.
