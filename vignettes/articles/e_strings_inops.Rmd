...
The `tinycodet` package includes 7 string arithmetic operators (3 of which re-exported from 'stringi'):
 ...
 - `e1 %s$% e2` provides access to `stri_sprintf`.
...
When supplying a list on the right hand side (see `s_pattern`), one can include the list element `at = "start"` or `at = "end"`:
...
The `strfind()<-` method locates, extracts, or replaces found patterns.
