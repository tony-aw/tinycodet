...
The primary use-case for this function is for loading functions inside a local environment, like the environment within a function. ("LL" stands for "Local & Locked".)
...
The `import_int()` function directly returns an internal function from a package. It is similar to the `:::` operator, but with 2 key differences:
 ...
 1) It it allows the user to explicitly set a library location through the `lib.loc` argument.
...
When importing packages with `tinycodet`' import system, S3 methods will work just fine. For example, the S3 method " `plot()` " works with objects from the `mgcv` R package, even when loaded in an alias:
...
# lib.loc and project isolation

All `import_` - functions have the `lib.loc` argument, that allows the user to specify exactly from which folder to take packages from. Thus, the import system from `tinycodet` allows for simple and straight-forward project isolation (and thus also version control), just like base R's `loadNamespace()` function.
...
... produces several messages, including the message "For conflicts report, packages order, and other attributes, run `attr.import(tdt.)`".

The `attr.import()` function allows the user to access the special attributes stored and locked inside the alias object. These attributes show which loaded package overwrites which loaded functions, in what order the packages are loaded and so on.

Here are some examples.

Show the packages loaded under the alias, and in which order the packages are loaded, and from which packages the re-exported functions came from:
...
Show the versions of the R packages loaded:

```{r}
attr.import(tdt., "versions")|> knitr::kable()
```
