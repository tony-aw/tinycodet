...
All `import_` - functions have the `lib.loc` argument, that allows the user to specify exactly from which folder to take packages from. Thus, the import system from `tinyoperations` allows for simple and straight-forward project isolation (and thus also version control), just like base R's `loadNamespace()` function.
