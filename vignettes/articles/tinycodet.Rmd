...
One can load a package without attaching the package (for example using `::`), or one can attach a package (for example using `library()` or `require()`). The advantages and disadvantages of loading without attaching a package versus attaching a package - at least those relevant for now - can be compactly presented in the following table:
...
What `tinycodet` attempts to do with its import system, is to somewhat find the best of both worlds. It does this by introducing the following functions:
 ...
 - `import_as()`: Allow a main package + its re-exports + its dependencies + its extensions to be loaded under a single alias. This essentially combines the attaching advantage of using multiple related packages (row 7 in table above), whilst keeping most advantages of using without attaching a package.
