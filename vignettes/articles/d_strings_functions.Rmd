...
The primary R-package that fixes this is 'stringi'. 'stringi' is arguably the fastest, most comprehensive, and one of the most downloaded string manipulation packages available at the time of writing. 'stringi' itself is dependency free. And many string related packages simply call 'stringi' functions.

For example: 'stringr' (from the 'tidyverse') is merely a thin wrapper around 'stringi', with less functionality and yet more dependencies.

The 'tinycodet' package actually adds more functionality to the 'stringi' string manipulation capabilities.
...
Although the functions in this R package were written in R, the functions - especially the string related ones - have been somewhat aggressively optimized. The string related functions, for example, are about in the same order of magnitude in terms of speed as the `stringi` functions they call.


Here some speed comparisons are given, using the [rbenchmark](https://cran.r-project.org/web/packages/rbenchmark/index.html) R package.
